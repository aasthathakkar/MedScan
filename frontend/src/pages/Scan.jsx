...
export default function Scan() {
  ...
  const clearFile = useCallback(() => {
    setFile(null); if (preview) URL.revokeObjectURL(preview); setPreview(''); setResult(null); setError('');
  ...
  return (
    <div className={s.page}>
      ...
      <Card className={`${s.uploadCard} ${dragging ? s.uploadCardDragging : ''}`}
        ...
        {!file ? (
          ...
          <div className={s.previewWrap}>
            ...
            <button className={s.clearBtn} onClick={e => { e.stopPropagation(); clearFile(); }} aria-label="Clear">×</button>
          </div>
        )}
        <input ref={inputRef} type="file" accept="image/*" className={s.hiddenInput} onChange={e => handleFile(e.target.files?.[0])}/>
      </Card>

      <Btn variant="primary" size="lg" className={s.scanBtn} disabled={!file||loading} onClick={handleScan}>
      ...
      {result && (
        <Card className={s.resultCard}>
          <h2 className={`${s.medicineName} ${!result.name ? s.medicineNameMuted : ''}`}>{result.name || 'Could not detect name'}</h2>
          <div className={s.expiryRow}>
            <span className={s.expiryLabel}>Expiry</span>
            <span className={`${s.expiryValue} ${!result.expiry ? s.expiryValueMuted : ''}`}>{result.expiry || 'Not found'}</span>
            <Badge variant={statusVariant[expiry.status]}>{statusLabel[expiry.status]}</Badge>
          </div>
          {expiry.days !== null && <p className={`${s.daysRemaining} ${expiry.status==='expired' ? s.daysRed : s.daysGreen}`}>{expiry.status==='expired' ? `Expired ${expiry.days} days ago` : `Expires in ${expiry.days} days`}</p>}
          <hr className={s.divider}/>
          {result.treats?.length > 0 && <><h3 className={s.sectionTitle}>Treats</h3><div className={s.pillsWrap}>{result.treats.map((t,i) => <span key={i} className={s.pill}>{t}</span>)}</div></>}
          {result.warnings?.length > 0 && <><h3 className={s.sectionTitle}>Warnings</h3><div className={s.warningsList}>{result.warnings.map((w,i) => <p key={i} className={s.warningItem}>⚠ {w}</p>)}</div></>}
          {result.ocr_text && <details className={s.ocrDetails}><summary className={s.ocrSummary}>View raw OCR text</summary><pre className={s.ocrText}>{result.ocr_text}</pre></details>}
          <Disclaimer/>
        </Card>
      )}
    </div>
