  ...
  return (
    <div className={s.page}>
      <section className={s.hero}>
        ...
        <p className={s.heroSubtext}>Check symptoms, scan expiry dates, and verify safety — all in one place.</p>
        ...
        <div className={s.statPills}>
          {stats.map(stat => <span key={stat} className={s.statPill}>{stat}</span>)}
        </div>
      </section>
      ...
      <section>
        ...
        <div className={s.featureGrid}>
          {features.map(f => (
            <Card key={f.to} className={s.featureCard} onClick={() => navigate(f.to)} role="button" tabIndex={0} onKeyDown={e => e.key === 'Enter' && navigate(f.to)}>
